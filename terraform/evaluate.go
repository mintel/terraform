...
type Evaluator struct {
	...
	// root module, passed in from external sources. This may be accessed
	// only while holding RootVariableValuesLock.
	RootVariableValues     map[string]*InputValue
	RootVariableValuesLock *sync.Mutex
}
...
type evaluationStateData struct {
	...
	// Resource, if non-nil, is the resource within ModulePath that
	// self-references will be resolved relative to. If not set, references
	// to "self" will produce errors.
	Resource *Resource
}
...
func (d *evaluationStateData) GetSelf(tfdiags.SourceRange) (cty.Value, tfdiags.Diagnostics) {
	panic("not yet implemented")
}
